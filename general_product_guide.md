...
**Primary:** agent-framework maintainers, and SaaS teams shipping per-customer agents — people already hand-rolling DPoP + delegation who recognize the moat in 30 seconds.
...
**20 steps covering:** Human signup → magic link → session → org creation → DCR agent registration → API keys → agent CRUD → `may_act` policies → vault provisioning → DPoP token issuance → depth-2 delegation → vault retrieval → depth-3 delegation → Stripe charge → audit log fetch → DPoP rotation → bulk revoke → vault disconnect → cascade revoke.

---

## 9. What Ships (v0.9.0 — Locked Features)
...
- Auth0 bcrypt→Argon2id transparent rehash on login
...
- Token vault (AES-256-GCM encrypted-at-rest, lazy refresh) — Google, GitHub verified; Slack, Microsoft, Notion experimental
...
- Embedded React admin dashboard — ~50 screens, ~1.1MB total component code
