# SharkAuth â€” General Product Guide
...
SharkAuth is an open-source OAuth 2.1 + OIDC authorization server that treats AI agents as first-class identities. It ships as a single ~30MB Go binary with embedded SQLite, an embedded React admin dashboard (~50 screens), Python + TypeScript SDKs, and a full CLI. It handles both human authentication (passkeys, MFA, SSO, magic links, RBAC, organizations) and agent authentication (DPoP-bound tokens, RFC 8693 delegation chains, token vault, cascade revocation) in one unified trust chain. `./shark serve` â€” running in 30 seconds, no Postgres, no Redis, no Docker required.
...
Auth0, Clerk, WorkOS, Stytch â€” they all built for the human-and-app world. Login, SSO, MFA, RBAC. Done. **That world is gone.**
...
4. One audit query, one `grant_id` â€” can we reconstruct every token, every hop, every resource touched?
...
- Bolt OAuth onto Auth0/Clerk â€” agents treated as generic M2M clients with no native delegation, no DPoP, no `act` claim awareness
...
- 30MB single binary
- Embedded SQLite â€” no Redis, no Postgres
...
**Primary:** agent-framework maintainers, and SaaS teams shipping per-customer agents â€” people already hand-rolling DPoP + delegation who recognize the moat in 30 seconds.
...
- Products like Cursor, Replit Agents, Lovable, Bolt, v0 â€” where each customer owns agents acting on their behalf
...
8. Vertical industry AI â€” legal (Harvey), medical (Hippocratic), finance (Hebbia)
...
The real moat = first-class lineage between customer â†’ agent â†’ token. Five precise responses at different blast radii:

| Layer | Threat | Response | Status |
|---|---|---|---|
| 1. Token | Agent's token leaks via prompt injection | Revoke token + refresh family (RFC 7009) | âœ… Ships |
| 2. Agent | One specific agent compromised | Kill all tokens for that agent | âœ… Ships |
| 3. Customer fleet | Customer churned or goes rogue | Cascade-revoke every agent they spawned | âœ… Ships |
...
Every token carries `cnf.jkt` â€” the SHA-256 thumbprint of the holder's public key. The resource server verifies the DPoP proof JWT on each request. **Steal the token, you get nothing.** The key never leaves the agent.
...
### 7c. `may_act_grants` â€” Real Rows, Not Vibes
```sql
...
```
Every token-exchange call writes the matching `grant_id` into the audit row. Operators can revoke by `id`, `from_id`, `to_id`, or in bulk. Grant lives outside the JWT â€” revocation takes effect without token rotation.
...
## 8. The Demo Story â€” Acme Travel Concierge
...
```
Maria (human)
â””â”€ Travel Concierge (master, client_credentials + DPoP)
   â”œâ”€ Flight Booker  (act_chain depth 2, vault: Amadeus)
   â”‚  â””â”€ Payment Processor  (act_chain depth 3, vault: Stripe)
   â”œâ”€ Hotel Booker   (act_chain depth 2, vault: Booking.com)
   â”œâ”€ Calendar Sync  (act_chain depth 2, vault: Google Calendar)
   â””â”€ Expense Filer  (act_chain depth 2, vault: Concur)
```

Every hop: DPoP proof. Every token: scoped down. Every action: one immutable audit row with full `act.act.sub` chain. Maria churns â†’ one call cascade-revokes the whole tree.

**20 steps covering:** Human signup â†’ magic link â†’ session â†’ org creation â†’ DCR agent registration â†’ API keys â†’ agent CRUD â†’ `may_act` policies â†’ vault provisioning â†’ DPoP token issuance â†’ depth-2 delegation â†’ vault retrieval â†’ depth-3 delegation â†’ Stripe charge â†’ audit log fetch â†’ DPoP rotation â†’ bulk revoke â†’ vault disconnect â†’ cascade revoke.

---

## 9. What Ships (v0.9.0 â€” Locked Features)

### Human Auth
- Email/password signup + login (Argon2id hashing)
- Passkey/WebAuthn login (FIDO2, discoverable + non-discoverable)
- Magic link login (email-based passwordless)
- Social OAuth â€” Google, GitHub, Apple, Discord
- MFA â€” TOTP with recovery codes
- Server-side sessions (encrypted cookies, `auth_method` tracking)
- Auth0 bcryptâ†’Argon2id transparent rehash on login
...
- Token vault (AES-256-GCM encrypted-at-rest, lazy refresh) â€” Google, GitHub verified; Slack, Microsoft, Notion experimental
- Webhooks with signature verification, retry, DLQ
- Audit log (filterable, exportable, retention-configurable)
- Auth flow engine (12 step types, conditional branching, webhooks, dry-run preview)
...
- ~252 backend routes wired into the chi router

### SDKs & UI
- Python SDK + TypeScript SDK (~75% endpoint coverage on agent platform + OAuth core)
- React component library: `@sharkauth/react`
- Embedded React admin dashboard â€” ~50 screens, ~1.1MB total component code
...
# user â†’ research-agent â†’ tool-agent
...
# act-claim records "Concierge â†’ Flight Booker" in JWT
```
...
# Maria â†’ Concierge â†’ Flight Booker â†’ Payment Processor
```
...
## 13. Admin Dashboard â€” ~50 Screens
...
## 14. Design System â€” ".impeccable"

**Brand personality:** Bold. Elegant. Fast. Monochrome. "No bullshit."

**Design principles:**
1. **Monochrome** â€” B&W foundation. Color ONLY for status meaning (success green, warn amber, danger red). No colored brand accents.
2. **Square** â€” Border radius capped at 3-5px. Sharp 90Â° corners everywhere.
3. **Dense** â€” Compact rows (~32-36px), 7-10px padding. Data over chrome.
4. Glamour
6. **Speed as UX** â€” Optimistic mutations, skeletons, keyboard shortcuts.

**Surfaces step:** `#000` â†’ `#0d0d0d` â†’ `#141414` â†’ `#1c1c1c`
...
|:-----------------------------------|:---------:|:-----:|:-----:|:--------:|
| Agent as first-class identity      | âœ…        | âŒ    | âŒ    | âŒ       |
| RFC 8693 token exchange + `act`    | âœ…        | partial | âŒ  | partial  |
| `may_act_grants` table, revocable  | âœ…        | âŒ    | âŒ    | âŒ       |
| `max_hops` enforced server-side    | âœ…        | âŒ    | âŒ    | âŒ       |
| RFC 9449 DPoP                      | âœ…        | âŒ    | âŒ    | partial  |
| `grant_id` correlated audit        | âœ…        | âŒ    | âŒ    | âŒ       |
| Cascade revocation (user â†’ agents) | âœ…        | âŒ    | âŒ    | âŒ       |
| Single binary, ~30MB               | âœ…        | âŒ    | âŒ    | âŒ (JVM, ~1GB) |
| Self-hostable, fully OSS           | âœ…        | âŒ    | âŒ    | âœ…       |
| Human auth (SSO, MFA, passkeys)    | âœ…        | âœ…    | âœ…    | âœ…       |
...
### SDKs
- Python: `shark_auth` package â€” `DPoPProver`, `OAuthClient`, `AgentsClient`, `AgentTokenClaims`
- TypeScript: `@sharkauth/sdk` â€” mirror of Python API surface
- React: `@sharkauth/react` â€” component library with hooks

---

## 19. The Pitch Demo â€” Blast Radius (4 Beats, 45-60s)
...
| 2. Open dashboard â†’ canvas | ~5s | Delegation canvas loads | Nothing. Silent navigation. |
| 3. Tree on screen | ~10s | MarÃ­a â†’ Concierge â†’ Flight Booker â†’ Payment Processor with scopes, timestamps, act-chain badges | Nothing. Let them read it. |
| 4. Point + revoke | ~15s | Hover Payment Processor â†’ click Revoke â†’ node greys out, others stay green | "This agent holds MarÃ­a's Stripe credentials. If it gets prompt-injected, I do thisâ€” *click* â€”and only it dies. Everything else keeps running." |

**Why this works:** Silence beats explanation. The tree IS the pitch. One sentence carries the moat. No jargon. Fast setup is the second moat.

---

## 20. Video B Script(Internal) â€” 90s Polished Launch Video

```
0:00-0:10  RAÃšL on camera, plain background.
...
0:10-0:25  CUT TO: shark serve â†’ first-boot magic-link â†’ dashboard home
           VO: "SharkAuth. Single binary, 30 seconds to running. Full
            human auth â€” magic-link, SSO, organizations. Plus the thing
            Auth0 cannot ship: agent identities."

0:25-0:50  CUT TO: register agent â†’ delegation chain â†’ vault retrieval (3rd hop fetches Gmail)
...
0:50-1:10  CUT TO: cascade revoke â€” admin runs revoke-agents on the user
...
1:10-1:30  BACK TO RAÃšL on camera
           "I'm RaÃºl. 18, solo, Monterrey, Mexico. 100,000 lines of
...
- **Color palette:** Monochrome â€” pure B&W with status-only color
...
1. **The Tree** â€” Delegation canvas showing MarÃ­a â†’ Concierge â†’ Flight Booker â†’ Payment Processor with DPoP badges and scope labels at each node
2. **The Click** â€” Single-node revoke on the canvas. Payment Processor greys out. Everything else stays green. Blast radius: one node.
3. **The Boot** â€” `shark serve` in terminal, 29 migrations apply, admin URL printed. 30 seconds from binary to running OIDC IdP.
4. **The Code** â€” 10-line Python snippet that Auth0 cannot replicate. DPoP token + delegation + chain walk + cascade revoke.
5. **The Audit** â€” One `grant_id`, full chain rendered: `[user] â†’ [agent-A] â†’ [agent-B]` with timestamps, scopes, jkt thumbprints.
6. **The Size** â€” `ls -lh ./shark` â†’ `~30MB`. Compare mentally to Keycloak's ~1GB JVM image.
...
- Branding tab â†’ ComingSoon
- Impersonate â†’ unwired
- Device flow (RFC 8628) â†’ returns 501
- Hosted login pages â†’ devs supply their own
- Proxy Rules tab in dashboard â†’ removed
- PyPI publish â†’ not yet (install via git clone)
- Auth Flow Builder UI â†’ backend works, UI deferred to v1.0
...
1. **Personal AI assistant on your inbox** â€” `may_act` grant from you to assistant-agent, scope `gmail:read`, expires 24h. Revoke from admin UI.
2. **Multi-agent orchestrator** â€” Planner â†’ research â†’ tool agent. `max_hops=3`, scopes downscoped per hop, every hop recorded.
3. **Internal employee + agent SSO** â€” Employees via OIDC/SAML, each spawns scoped agents.
4. **MCP server auth** â€” Register MCP server as application, issue per-consumer client credentials, gate tool calls behind DPoP-bound tokens.
5. **Embedded auth for open-source SaaS** â€” Drop binary next to your app, point at it for OAuth + OIDC + admin UI.
6. **Compliance-grade audit** â€” Run proxy in front of APIs, get DPoP enforcement and `grant_id` audit on existing identities.
