...
| 2026-04-20T21:45:00Z | T21 | done | subagent-F | 77920c3 — Matrix view added to rbac.tsx (top-level ?view=roles|matrix); grid rows=permissions (alphabetical) × cols=roles, checkbox cells w/ optimistic toggle + rollback toast, filter input, col-header bulk grant/revoke w/ confirm, Used-by-N-users per row from batch-usage. Matrix extracted to rbac_matrix.tsx. Backend routes all present (verified router.go:356-376); no backend work needed. Typecheck pass. |
| 2026-04-20T21:50:00Z | T15 | done | subagent-E | eaa528f (backend) + c2dd599 (frontend login.tsx bundled w/ T21 backfill due to concurrent stage). Consume route POST /admin/bootstrap/consume no-auth, 32-byte crypto/rand token, sha256 hash in-memory single-use 10-min expiry, audit admin.bootstrap.consumed on success. Startup print only when audit has zero admin.* events. Frontend: ?bootstrap=<tok> auto-POST on mount, strip param, "[?] Where is my key?" inline hint with shark admin-key show. |
| 2026-04-20T22:00:00Z | T06 | dispatch+kill | subagent-G | killed pre-edit by user. Gotcha surfaced: no admin POST create-invitation endpoint exists (only session-auth at router.go:341). Three resume options in DASHBOARD_DX_NEXT.md. Tree clean. |
| 2026-04-20T22:00:00Z | T16 | dispatch+kill | subagent-H | killed pre-edit by user. Tree clean. Brief ready in DASHBOARD_DX_SUBAGENT_BRIEFS.md. |
| 2026-04-20T22:02:00Z | PAUSE | handoff | claude-opus-4-7 | 22/24 done. Remote at 62faa3e. Next up: T06 + T16 then T24 ship review. See DASHBOARD_DX_NEXT.md. |
| 2026-04-20T21:52:00Z | T15-smoke | claim | subagent-E | smoke section 71 — 400 empty, 400 missing token, 401 random, 401 handler-level (not mw) |
