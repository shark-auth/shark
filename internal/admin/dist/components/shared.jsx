// Shared icons, helpers, and primitives for SharkAuth dashboard
// All components attached to window for cross-file Babel scope.
...
// Shark logo — real asset. Icon-only (shark glyph), dark rounded tile.
function SharkIcon({ size = 22 }) {
  return (
    <div style={{
...
// Full "Sharky" wordmark — 2:1 rectangle, white shark + wordmark on black.
function SharkFullLogo({ width = 200 }) {
  return (
    <img
      src="assets/sharky-full.png"
      alt="Shark"
...
// simple hash-based avatar color
...
function Avatar({ name, email, size = 22, agent = false }) {
...
function CopyField({ value, mono = true, truncate = 24 }) {
  const [copied, setCopied] = React.useState(false);
  const display = truncate && value.length > truncate ? value.slice(0, truncate - 4) + '…' + value.slice(-3) : value;
  return (
    <button
      className={mono ? 'mono' : ''}
      onClick={(e) => { e.stopPropagation(); setCopied(true); setTimeout(() => setCopied(false), 900); }}
      style={{
...
// Sparkline
function Sparkline({ data, height = 28, color = 'var(--fg)' }) {
  const w = 120;
...
// Donut chart
function Donut({ segments, size = 92, thickness = 14 }) {
...
// Keyboard sequence hint
function Kbd({ keys }) {
  return <span className="row" style={{gap:3}}>{keys.split(' ').map((k, i) => <span key={i} className="kbd">{k}</span>)}</span>;
}

Object.assign(window, { Icon, SharkIcon, SharkFullLogo, Avatar, CopyField, Sparkline, Donut, Kbd, hashColor });
