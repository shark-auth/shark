...
func (s *Server) MintBootstrapToken(ctx context.Context) (string, error) {
	...
	for _, l := range logs {
		if l.ActorType == "admin" || strings.HasPrefix(l.Action, "admin.") {
			return "", nil
		}
	}
...
// A successful read consumes the plaintext file immediately. This endpoint is
// public by design for first-run UX, so the server must enforce "show once"
// globally, not per browser/localStorage.
//
...
func (s *Server) handleFirstbootKey(w http.ResponseWriter, r *http.Request) {
	...
	hasRealKeys := false
	for _, k := range keys {
		// Ignore the default admin key generated by RunFirstBoot
		if k.Name != "default-admin" {
			hasRealKeys = true
			break
		}
	...
	if err := os.Remove(keyPath); err != nil && !os.IsNotExist(err) {
		writeJSON(w, http.StatusInternalServerError, map[string]string{
			"error":   "internal_error",
			"message": "Failed to consume first-boot key.",
		})
		return
	}
