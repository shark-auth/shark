...
**Goal:** Ship `tools/agent_demo_concierge.py` — interactive 20-step demo exercising every major shark feature in a single coherent travel-booking narrative, ending with depth-3 delegation chain visible in dashboard.
...
| 16 | Audit log fetch — render full ASCII tree of chain + metadata | Audit specificity (just shipped) |
| 17 | Rotate Flight Booker DPoP key mid-flight (Layer 4 / Method 10) | DPoP rotation |
| 18 | Bulk-revoke `tc_payment_*` pattern | Layer 4 bulk |
| 19 | Disconnect Stripe vault → Payment Processor token revoked, others survive | Layer 5 cascade |
| 20 | Cascade-revoke Maria → all agents + vault tokens wiped | Layer 3 cascade |

---

## Phase A: Investigation (parallel, sonnet)

- [ ] Sonnet `/investigate` found acr/amr emission absent → MFA step-up dropped from demo, backlog post-launch.
...
## Phase C: Chain depth + cascades — steps 14-20 (sonnet subagent #2, worktree)

- [ ] Steps 14-15: depth-3 token-exchange + Stripe charge
- [ ] Step 16: ASCII tree renderer of audit chain
- [ ] Step 17-19: rotation + bulk revoke + vault disconnect
- [ ] Step 20: full cascade
- [ ] Cleanup function (only on natural finish; Ctrl-C handler that prints "leaving state for inspection" + exits)
- [ ] E2E run — all 20 steps green
...
- **What you just saw** — per-step plain English (not the 20-row table; 6-8 paragraphs grouped by phase: signup → spawn agents → delegate → cascade)
- **The moat** —

  > **Your agents are already doing this. They're just not doing it safely.**
  >
  > Sub-agents, vault, delegation chains, cascade revoke — these are not optional features for production agents. Every team shipping AI agents has them. The question is whether you're rolling your own (and getting it wrong), or using shark.
  >
  > Without shark: 3 months of OAuth + DPoP + RFC 8693 + audit infra + cascade-revoke schema design. Without shark: token theft is a P0 incident.
  >
...
- [ ] ASCII delegation tree at step 16 mirrors Railway canvas in dashboard
- [ ] ANSI colors for OH-SHIT moments (chain depth, cascade)
...
```
Phase A (investigate) ──> Phase B (steps 1-13) ──> Phase C (steps 14-20) ──> Phase E (polish + commit)
```

Phase A ran in background while plan was finalised; verdict was acr/amr emission absent, so MFA step-up was dropped from the demo entirely.
